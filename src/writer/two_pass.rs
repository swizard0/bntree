...
pub struct PassWrite<BW, O> {
    pub op: OpWrite<O>,
    pub next_fold: fold::Instruction<BW>,
}
...
enum Fsm<O> {
    ...
    BusyMarkup(BusyMarkup<O>),
    BusyWrite(BusyWrite<O>),
}
...
impl<O> Script<O> {
    pub fn start(tree_offset: O, tree_header_size: O) -> Script<O> {
        Script { inner: Fsm::Init { tree_offset, tree_header_size }, }
    }

    pub fn step<'s, BA, BW>(self, arg: StepArg<'s, BA, BW, O>) -> Result<Instruction<BA, BW, O>, Error> {
...
pub struct InitialLevelSize<O> {
    pub level_index: usize,
    pub next: InitialLevelSizeNext<O>,
}

pub struct InitialLevelSizeNext<O> {
    busy: BusyMarkup<O>,
    level_index: usize,
}
